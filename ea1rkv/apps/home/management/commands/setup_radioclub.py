...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if home is None:
            ...
            if (
                starter.specific_class is not Page
                or starter.slug != "home"
                or starter.get_children().exists()
                or HomePage.objects.exists()
            ):
                raise CommandError(
            ...
            if root.get_children().filter(slug=slug).exists():
                raise CommandError(
                    "Ya existe una página con slug inicio; revísala en Wagtail."
                )
            home = root.add_child(
                instance=HomePage(
                    title="Inicio",
        ...
        if blog is None:
            if (
                home.get_children().filter(slug="blog").exists()
                or BlogIndexPage.objects.exists()
            ):
                raise CommandError(
                    "Ya existe otro blog o una página con slug blog. Revísala en Wagtail."
                )
            blog = home.add_child(
        ...
        for model, title, slug, intro in (
            (
                ...
                "Servicios",
                "servicios",
                "<p>Repetidores, frecuencias, equipos y otros recursos del radioclub. Consulta la ficha de cada recurso para conocer sus datos y condiciones de uso.</p>",
            ),
            (
                CallsignsPage,
                "Indicativos especiales",
                "indicativos-especiales",
                "<p>Información, fechas y fotografías de las actividades con indicativos especiales del radioclub.</p>",
            ),
        ):
            if not model.objects.child_of(home).exists():
                if home.get_children().filter(slug=slug).exists():
                    raise CommandError(
                        f"Ya existe una página con slug {slug}. Revísala antes de crear la sección."
                    )
        ...
        if hostname:
            if not re.fullmatch(r"[a-z0-9.-]+", hostname):
                raise CommandError(
                    "WAGTAIL_SITE_HOSTNAME debe contener sólo el dominio."
                )
            try:
                port = int(settings.WAGTAIL_SITE_PORT)
            except ValueError as error:
                raise CommandError("WAGTAIL_SITE_PORT debe ser un número.") from error
            if not 1 <= port <= 65535:
                raise CommandError("WAGTAIL_SITE_PORT está fuera de rango.")
